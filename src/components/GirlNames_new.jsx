...
const GirlNames = () => {
  ...
  const renderSubcategoryContent = (subcategory) => {
    if (subcategory === 'zodiac' && !selectedZodiac) {
      return (
    ...
    if (selectedZodiac) {
      const names = zodiacNamesData[selectedZodiac.name.toLowerCase()] || [];
      return (
        <Box>
          <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            >
              <span style={{ fontSize: '2rem' }}>{selectedZodiac.symbol}</span>
              {selectedZodiac.name} Names
            </Typography>
          </Box>
          <Grid container spacing={3}>
